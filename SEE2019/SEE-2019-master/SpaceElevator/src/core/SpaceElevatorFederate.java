...
public class SpaceElevatorFederate extends SEEAbstractFederate implements Observer{
	...
	private SpaceElevator spaceElevator = null;
	private SpaceElevatorLauncher launcher = null;
	private LauncherStates launcherState = LauncherStates.IDLE;
	
	private boolean control = true;
	
	public SpaceElevatorFederate(SEEAbstractFederateAmbassador seefedamb, SpaceElevator spaceElevator, 
			SpaceElevatorLauncher launcher) {
		super(seefedamb);
		this.launcher = launcher;
		this.spaceElevator = spaceElevator;
	...
	IllegalAccessException, NameNotFound, InvalidObjectClassHandle, AttributeNotDefined, ObjectClassNotDefined, SubscribeException, InterruptedException, UnsubscribeException, InvalidInteractionClassHandle, InteractionClassNotDefined, InteractionClassNotPublished, InteractionParameterNotDefined, PublishException {
		...
		if(!SynchronizationPoint.INITIALIZATION_STARTED.isAnnounced()){
			...
			try {
				super.publishElement(spaceElevator, spaceElevator.getName());
	...
	public void update(Observable arg0, Object arg1) {
		System.out.println("The Space Elevator has received an update");
	...
	protected void doAction() {
		...
		try {
			...
			super.updateElement(this.spaceElevator);
		...
		System.out.println("Space Elevator: " + spaceElevator);
	...
	public void changeControle() {
		control = !control;
		System.out.println("---------------------- Space Elevator ------------------------------");
