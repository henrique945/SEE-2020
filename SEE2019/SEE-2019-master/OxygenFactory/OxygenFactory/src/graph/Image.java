		...
		for (int x = 0; x < image.getWidth(); x++) {
			...
			for (int y = 0; y < image.getHeight(); y++) {
				...
				if (mycolor.equals(Color.BLUE)) {
					pixels[x][y] = -5;
					PositionOfOxygenFactory.setPosition("south", x, y);
				}

				if (mycolor.equals(Color.red)) {
					pixels[x][y] = -6;
					PositionOfOxygenFactory.setPosition("west", x, y);
				}

				if (mycolor.equals(Color.green)) {
					pixels[x][y] = -7;
					PositionOfOxygenFactory.setPosition("north", x, y);
				}

				if (mycolor.equals(Color.yellow)) {
					pixels[x][y] = -8;
					PositionOfOxygenFactory.setPosition("east", x, y);
				}

				if (mycolor.equals(Color.magenta)) {
					pixels[x][y] = -9;
					PositionOfOxygenFactory.setPosition("o2fac", x, y);
				}
